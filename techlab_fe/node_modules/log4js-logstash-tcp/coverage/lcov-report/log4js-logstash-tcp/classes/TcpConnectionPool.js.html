...
<head>
    <title>Code coverage report for log4js-logstash-tcp/classes/TcpConnectionPool.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../index.html">All files</a> / <a href="index.html">log4js-logstash-tcp/classes</a> TcpConnectionPool.js</h1>
        <div class='clearfix'>
...
&nbsp;
        ...
        this.connection.on("timeout", <span class="fstat-no" title="function not covered" >()</span> =&gt; {<span class="cstat-no" title="statement not covered" >self.destroy()}</span>);
        this.connection.on("close", () =&gt; {self.destroy()});
        this.connection.on("error", <span class="fstat-no" title="function not covered" >(e</span>rr) =&gt; {
<span class="cstat-no" title="statement not covered" >            console.error(`logstash Error with the connection to ${host}:${port}. ${err}`);</span>
...
<span class="cstat-no" title="statement not covered" >                return obj + "\n";</span>
            } else {
...
&nbsp;
    constructor(config) {
        this.tcpConnections = {}
        <span class="missing-if-branch" title="if path not taken" >I</span>if(config) <span class="cstat-no" title="statement not covered" >this.config = config;</span>
        else this.config = {};
&nbsp;
        <span class="missing-if-branch" title="else path not taken" >E</span>if(!this.config.maxTcpConnections) {
...
&nbsp;
        let completed = 0;
        const complete = () =&gt; {
            <span class="missing-if-branch" title="else path not taken" >E</span>if (++completed &gt;= connections.length) {
